...
@section('content')
    ...
    <div class="container my-5" style="max-width:480px; font-family: 'Poppins', sans-serif;">
        <div class="shadow rounded-4 p-4 bg-white" style="border: 1px solid #e3e6f0;">
            <h3 class="mb-4 text-center text-primary fw-bold">Form Presensi Izin Siswa zzzzzzzzzz</h3>
            ...
            @if ($data)
                <div class="mb-4 p-3 rounded-3" style="background: #f0f5ff; border-left: 5px solid #3b82f6;">
                    ...
                    <p class="mb-0" style="font-size: 1.1rem; font-weight:700; color:#1e40af;">
                        {{ $data->nama_siswa ?? 'Nama tidak tersedia' }}</p>
                    <small class="text-muted">Kelas: {{ $data->kelas ?? '-' }} | Jurusan: {{ $data->jurusan ?? '-' }}</small>
                </div>
            @endif
            ...
            <form action="{{ route('ortu.kirimPresensi') }}" method="POST" enctype="multipart/form-data" novalidate>
                ...
                <div class="mb-4">
                    <label for="tanggal" class="form-label fw-semibold" style="letter-spacing: 0.04em;">Tanggal Izin <span
                ...
                <div class="mb-4">
                    <label for="keterangan" class="form-label fw-semibold" style="letter-spacing: 0.04em;">Keterangan
                        (Opsional)</label>
                ...
                <div class="mb-5">
                    <label for="foto" class="form-label fw-semibold" style="letter-spacing: 0.04em;">Unggah Foto Bukti Izin
                ...
                <div class="d-flex justify-content-between">
                    <a href="{{ route('ortu.dashboard') }}" class="btn btn-outline-primary fw-semibold px-4 py-2 rounded-3"
                        style="flex: 1; margin-right: 0.75rem; transition: background-color 0.3s ease, color 0.3s ease;">
                        Kembali
                    </a>
                    <button type="submit" class="btn btn-primary fw-semibold px-4 py-2 rounded-3"
                        style="flex: 1; transition: box-shadow 0.3s ease;">
                        Kirim Izin
                    </button>
                </div>
            </form>
